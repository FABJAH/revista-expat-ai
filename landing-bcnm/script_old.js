...
function openServiceChat(service) {
    ...
    currentService = service; // Guardar el servicio actual
    ...
    // Scroll a la sección de respuestas
...
async function sendMessage() {
    ...
    try {
        ...
        if (data.json && data.json.opciones && data.json.opciones.length > 0) {
            ...
            if (currentService) {
                // Si viene de un servicio, mostrar en la sección de respuestas
                showServiceResponses(data);
                currentService = null; // Limpiar el servicio actual
            } else {
                // Si viene del chat general, mostrar en el chat
